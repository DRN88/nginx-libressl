...
%define BASE_CONFIGURE_ARGS $(echo "--prefix=%{_sysconfdir}/nginx --sbin-path=%{_sbindir}/nginx --modules-path=%{_libdir}/nginx/modules --conf-path=%{_sysconfdir}/nginx/nginx.conf --error-log-path=%{_localstatedir}/log/nginx/error.log --http-log-path=%{_localstatedir}/log/nginx/access.log --pid-path=%{_localstatedir}/run/nginx.pid --lock-path=%{_localstatedir}/run/nginx.lock --http-client-body-temp-path=%{_localstatedir}/cache/nginx/client_temp --http-proxy-temp-path=%{_localstatedir}/cache/nginx/proxy_temp --http-fastcgi-temp-path=%{_localstatedir}/cache/nginx/fastcgi_temp --http-uwsgi-temp-path=%{_localstatedir}/cache/nginx/uwsgi_temp --http-scgi-temp-path=%{_localstatedir}/cache/nginx/scgi_temp --user=%{nginx_user} --group=%{nginx_group} --with-file-aio --with-threads --with-ipv6 --with-http_addition_module --with-http_auth_request_module --with-http_dav_module --with-http_flv_module --with-http_gunzip_module --with-http_gzip_static_module --with-http_mp4_module --with-http_random_index_module --with-http_realip_module --with-http_secure_link_module --with-http_slice_module --with-http_ssl_module --with-http_stub_status_module --with-http_sub_module --with-http_v2_module --with-mail --with-mail_ssl_module --with-stream --with-stream_ssl_module --with-openssl=%{_sourcedir}/portable-%{libressl_version} --with-ld-opt=-lrt --add-module=%{_sourcedir}/ngx_pagespeed-latest-stable")
...
%prep
cd %{_sourcedir}
if [ ! -f %{_sourcedir}/v%{libressl_version}.tar.gz ]; then wget "https://github.com/libressl-portable/portable/archive/v%{libressl_version}.tar.gz" -O %{_sourcedir}/v%{libressl_version}.tar.gz; fi
tar -xzf v%{libressl_version}.tar.gz
cd portable-%{libressl_version}
./autogen.sh

cd %{_sourcedir}
if [ ! -f %{_sourcedir}/latest-stable.tar.gz ]; then wget "https://github.com/pagespeed/ngx_pagespeed/archive/latest-stable.tar.gz" -O %{_sourcedir}/latest-stable.tar.gz; fi
tar -xzf latest-stable.tar.gz
cd ngx_pagespeed-latest-stable
if [ ! -f %{_sourcedir}/1.11.33.4.tar.gz ]; then wget "https://dl.google.com/dl/page-speed/psol/1.11.33.4.tar.gz"; fi
tar -xzf 1.11.33.4.tar.gz # psol
